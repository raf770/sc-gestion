...
type Produit = { id: string; ref: string; nom: string; prixVente: number; prixAchat: number; tva: number; stock: number; stockMin: number; categorie?: string; ecoDeee: number; ecoDea: number }
...
function ProduitsList() {
  const [produits, setProduits] = useState<Produit[]>([]); const [total, setTotal] = useState(0); const [q, setQ] = useState(''); const [page, setPage] = useState(0); const [showForm, setShowForm] = useState(false); const [editProd, setEditProd] = useState<Produit | null>(null); const limit = 50
