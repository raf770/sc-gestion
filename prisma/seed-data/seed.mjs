...
async function main() {
  ...
  const produits = JSON.parse(readFileSync(join(__dirname, 'produits.json'), 'utf-8'))
  let prodCount = 0
  ...
  for (let i = 0; i < clients.length; i += 500) {
    ...
    await prisma.client.createMany({
      data: batch.map(c => ({ id: 'cli_' + (c.ref || cid++), ref: c.ref || null, nom: c.nom, email: c.email || null, telephone: c.telephone || null, adresse: c.adresse || null, ville: c.ville || null, cp: c.cp || null, pays: c.pays || 'France', siret: c.siret || null })),
      skipDuplicates: true,
    })
  ...
  const factures = JSON.parse(readFileSync(join(__dirname, 'factures.json'), 'utf-8'))
  ...
  for (const f of factures) {
    ...
    try {
      await prisma.document.create({ data: { id: 'doc_' + f.numero, type: 'facture', numero: f.numero, clientId, date: dateVal, echeance: echVal, status: f.status || 'brouillon', entityId: 'sc_mobilier', isProforma: f.isProforma || false, totalHT: ht, totalTVA: tva, totalTTC: ttc, restePayer: f.restePayer || 0 } })
      facCount++
    } catch(e) { facSkipped++ }
  }
